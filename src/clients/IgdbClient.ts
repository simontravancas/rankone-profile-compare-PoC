...
export default class IgdbClient {
  private static async request(endpoint: string, query: string) {
    ...
    const gameInfo: any = axios
      .post(
        `http://127.0.0.1:8080/https://api.igdb.com/v4/${endpoint}/`,
        query,
        {
          headers: {
  ...
  static async getGamesInfoByArray(gameIdArray: Array<string>) {
    const igdbIdCommaSeparatedString = gameIdArray.reduce(
      (prev: string, next: string) => {
        return prev + ',' + next
      }
    )
    console.log(
      'igdbIdCommaSeparatedString',
      igdbIdCommaSeparatedString,
      'gameIdArray.length + 1',
      gameIdArray.length + 1
    )
    const query = `fields name,genres.name,themes.name; where id = (${igdbIdCommaSeparatedString}); limit ${
      gameIdArray.length + 1
    };`
